...
const airtableTables = [
  { table: 'Countries', filename: 'scripts/countries.json' },
  { table: 'Authorities', filename: 'scripts/authorities.json' },
  { table: 'Policy tracker', filename: 'scripts/policy-tracker.json' },
  { table: 'Vaccines', filename: 'scripts/vaccines.json' },
  { table: 'VaccineCountryEvents', filename: 'scripts/vaccine-country-events.json' },
  { table: 'Authority Review Events', filename: 'scripts/authority-review-events.json' },
  { table: 'PolicyTrackerListFields', filename: 'scripts/policy-tracker-select-choices.json' }
]
...
const downloadTableToFile = async (base, options) => {
  ...
  base(options.table)
    ...
    .eachPage(function page (records, fetchNextPage) {
      ...
      const outputString = JSON.stringify(airtableRecords, null, 2)
